...
climbing.py v6 : algorithme slope-first, paramètres réglables dans la sidebar.
"""
...
def creer_figure_meteo(resultats):
    """3 graphiques empilés : Température / Vent+Rafales / Pluie."""
...
def main():
    ...
    # ── PARAMÈTRES DE DÉTECTION DES ASCENSIONS ───────────────────────────────
    st.sidebar.divider()
    with st.sidebar.expander("🏔️ Détection des montées", expanded=False):
        ...
        seuil_debut = st.slider(
            "Seuil de départ (%)",
            min_value=0.5, max_value=5.0,
            value=float(climbing_module.SEUIL_DEBUT),
            step=0.5,
            help="Pente minimale pour démarrer une montée. "
                 "Baisser = démarre plus tôt. Monter = ignore les faux-plats."
        )
        seuil_fin = st.slider(
            "Seuil de fin (%)",
            min_value=0.0, max_value=3.0,
            value=float(climbing_module.SEUIL_FIN),
            step=0.5,
            help="Pente en dessous de laquelle la montée est terminée."
        )
        fusion_m = st.slider(
            "Fusion (D− max entre 2 montées, m)",
            min_value=10, max_value=200,
            value=int(climbing_module.MAX_DESCENTE_FUSION_M),
            step=10,
            help="Deux montées séparées par moins que cette valeur sont fusionnées."
        )
        # Injecter les valeurs dans le module pour ce run
